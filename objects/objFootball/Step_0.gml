/// @desc objFootball – Step (arc & landing, any eligible receiver can catch)
...
if (global.play_active && attached_to == noone)
{
    ...
    // === CATCH CHECK: WR or RB ===
    var catcher = noone;
    // Check for WR catch first
    var wr_hit = collision_circle(x, y, 14, objWRParent, false, false);
    if (wr_hit != noone && !wr_hit.hasBall) catcher = wr_hit;
    // If no WR, check for RB catch (objRunningBacks parent)
    if (catcher == noone) {
        var rb_hit = collision_circle(x, y, 14, objRunningBacks, false, false);
        if (rb_hit != noone && !rb_hit.hasBall) catcher = rb_hit;
    }
    // If caught by anyone
    if (catcher != noone)
    {
        with (catcher)
        {
            ...
            hasBall     = true;
            run_timer   = 0;
            // Use running with ball sprite
            if (variable_instance_exists(self, "sprRunWithBallgry"))
        ...
        attached_to            = catcher.id;
        global.player_has_ball = catcher.id;
